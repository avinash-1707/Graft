...
export interface WidgetAuthPluginOptions {
  db: Database;
}
...
/**
 * Decorates `validateWidget`: resolves the public embed token to an org and
 * enforces the per-org Origin allow-list (the real boundary — the token is public).
 * Shared between the gateway (public ingress) and ai-service (until gateway
 * downstream routing forwards the resolved identity). Sets `request.widgetOrg`.
 */
const widgetAuthPlugin: FastifyPluginAsync<WidgetAuthPluginOptions> = async (app, opts) => {
  ...
  app.decorate('validateWidget', async (request: FastifyRequest) => {
    ...
    if (!token) throw AuthErrors.invalidEmbedToken();

    const org = await findOrganizationByEmbedToken(opts.db, token);
    if (!org) throw AuthErrors.invalidEmbedToken();
    ...
    if (!origin) throw AuthErrors.originNotAllowed();

    const allowed = await isOriginAllowed(opts.db, org.id, origin);
    if (!allowed) throw AuthErrors.originNotAllowed();

    request.widgetOrg = { organizationId: organizationIdSchema.parse(org.id) };
  });
};

export default fp(widgetAuthPlugin, { name: 'graft-widget-auth' });
