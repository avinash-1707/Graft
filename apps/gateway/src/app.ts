...
import { buildDodoConfig } from './billing/service.js';
...
export async function buildApp(opts: BuildAppOptions): Promise<FastifyInstance> {
  ...
  const dodo = buildDodoConfig(env);
  if (!dodo) logger.warn('Dodo Payments not configured — billing checkout/webhooks disabled');
  await app.register(billingRoutes, { db, env, dodo });
  await app.register(billingWebhookRoutes, { db, dodo, logger });
